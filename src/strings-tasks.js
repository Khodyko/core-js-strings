...
function isString(value) {
  return (
    (value &&
      (typeof value === 'string' || typeof value.valueOf() === 'string')) ||
    value === ''
  );
}
...
function removeLastOccurrences(str, value) {
  if (str.indexOf(value) >= 0) {
    return (
      ...
      str.substring(str.lastIndexOf(value) + value.length, str.length + 1)
    );
  }
  return str;
}
...
function isPalindrome(str) {
  const strStr = str.replace(/[^A-Za-z0-9]/g, '').toLowerCase();
  return strStr === strStr.split('').reverse().join('');
}
